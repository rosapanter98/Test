...
using AvaloniaApplication3.Services.Interfaces; // keep if you later swap to an IFilePickerService
...
namespace AvaloniaApplication3.ViewModels.Admin
{
    public partial class QuizManagementViewModel : ViewModelBase
    {
        ...
        // ---------------- Commands (generated by MVVM Toolkit) ----------------
                ...
                Filters =
        {
            ...
            var paths = await dialog.ShowAsync(window);
            ...
            try
            {
                // NOTE: match your service signature
                var id = await _quizImportService.ImportFromFileAsync(path, overwriteExisting: true);

                await LoadQuizzesAsync();
                var imported = Quizzes.FirstOrDefault(q => q.Id == id);
